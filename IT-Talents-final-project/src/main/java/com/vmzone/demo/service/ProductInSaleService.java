...
/**
 * Service layer communicating with product repository and product in sale repository for managing requests for product in sale
 * 
...
public class ProductInSaleService {
	...
	public List<ListProductsInSale> showProductsInSale() {
		...
		return this.productInSaleRepository.findAll().stream()
				.filter(product -> product.getProduct() != null
				&& product.getSaleId() != null 
				&& product.getStartDate().isBefore(dateNow)
				&& product.getEndDate().isAfter(dateNow))
				.map(p -> new ListProductsInSale(p.getProduct().getTitle(), p.getProduct().getInformation(), p.getStartDate(),
						p.getEndDate(), p.getDiscountPercentage()))
	...
	public void deleteExpiredPromotions() {
		LocalDateTime dateNow = LocalDateTime.now();
		this.productInSaleRepository.deleteByEndDateLessThan(dateNow);
	}
	
	public void deletePromotion(long id) {
		this.productInSaleRepository.deleteById(id);
	}
